...
public sealed class VideoSubtitleService(ITranslationService translationService, IAppLogger logger) : IAsyncDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!File.Exists(whisperModelPath)) throw new OfflineEngineException("请选择存在的 Whisper GGML 模型文件。");
        _source = source;
        _target = target;
        ...
        _factory = WhisperFactory.FromPath(whisperModelPath);
        ...
        StatusChanged?.Invoke(this,
            $"低延时识别中：静音触发切片，最长 {MaximumChunkSeconds:F1} 秒送识别，译文晚到也会回填到字幕。");
