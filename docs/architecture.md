...
- `SpeechRecognitionRouter` 在开始采集前校正不兼容组合：English 可继续使用 Parakeet，自动检测/中文/日语会优先路由到已启用的 SenseVoice，并以 Whisper 作为多语言后备。
...
- 文本语言分析保留汉字、假名、拉丁字母的脚本比例；同目标语言的纯文本直接生成字幕，中英混合文本使用 `AutoDetect` 翻译协议统一成目标语言，避免把英文残留在中文字幕中。
