...
- `LocalTranslator.Infrastructure`：OCR、屏幕与音频采集、Whisper、LLamaSharp、Provider、安全配置和 SRT。
...
- 默认字幕模型由 `SpeechModelManager` 下载到应用数据目录，并在安装前校验文件长度与 SHA-256；失败或取消时清理临时文件。
- 实时字幕按 WASAPI 设备真实混音格式采集，经立体声转单声道和 WDL 重采样后统一为 16 kHz PCM；使用低延时静音触发切片、约 160ms 重叠、静音/低置信度过滤和容量为 2 的丢旧保新队列。
- Whisper 输出先进入语义缓冲：连续短片段即时更新原文，但只有遇到标点、停顿、时间或长度阈值才提交给翻译模型，避免碎片句被断章误译。
...
- Whisper 结果进入语言检测和翻译队列前会过滤完整括号包裹的字幕署名、caption/subtitle credit 和网址型幻觉。
